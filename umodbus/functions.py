...
def write_multiple_registers(starting_address: int,
                             ...
                             signed: bool = True) -> bytes:
    """
    Create Modbus message to update multiple registers
...
def validate_resp_data(data: bytes,
    ...
    if function_code in [Const.WRITE_SINGLE_REGISTER]:
        resp_addr, resp_value = struct.unpack(fmt, data)

        if (address == resp_addr) and (value == resp_value):
            return True
    elif function_code in [Const.WRITE_MULTIPLE_REGISTERS]:
...
def response(function_code: int,
    ...
    :rtype:     bytes
    """
    if function_code in [Const.READ_HOLDING_REGISTERS,
                           Const.READ_INPUT_REGISTER]:
    ...
    elif function_code in [Const.WRITE_SINGLE_REGISTER]:
